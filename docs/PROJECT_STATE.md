...
> - **Itération #6 (calibration walk-forward systématique)** : ~25 configurations testées une à une, mesure walk-forward à chaque étape. **Découverte clé** : `min_score=45` (vs 25 baseline) + `require_uptrend_for_buy=True` transforment la stratégie. Champion mesuré : Sharpe **+0.93** (vs -0.91), PF **3.03** (vs 0.83), 4/10 fenêtres positives (vs 0/10). **Critère #8 PF>1.3 atteint** ; critère #7 Sharpe>1 à 0.07 près. Backtest UI mis à jour avec config champion.
> - **Itération #7 (fine-tuning + tests robustesse)** : tests sur 5 univers de coins (4→8 coins) → **diversifier dégrade le Sharpe** (8 coins → -0.18). Tests sur 6 granularités (1h, 2h, 4h, 6h, 12h, 1d) → **4h reste optimal** (toutes les autres < 0). Fine-tuning paramètres (min_score 40-46, min_rr 1.5-2.0, cooldown 4-18) → **plateau stable** confirmé. Config champion validée comme optimum local de la stratégie multi-indicateurs actuelle. **Pour Sharpe > 1.0 et consistency > 60 %, refonte du scoring requise** (changer pondérations RSI/MACD/EMA, ajouter volume confirmation, pullback detection) — chantier suivant.
